...
int main(int argc, const char **argv) {
  ...
  if(cf.findClass({"Fecha"})) {
    cf.staticVariable("Fecha", {"AñoMaximo", "AñoMinimo"}, {"const", "const"},
		     "Revisa el enunciado respecto a AñoMinimo y AñoMaximo.");
    cf.mutableVariable("Fecha", {"crep", "actual"},
		     "Revisa el enunciado respecto a los atributos crep y actual.");	 
    ...
    vector<string> initializationList = {"int", "int", "int", "_Bool"};
    ...
    vector<string> functionNames = {"sprintf", "sscanf"};
    ...
    cf.method({"cadena"}, {{"?"}}, "Fecha", {"noconst"}, {false},
        "Revisa si tienes un método u operador que debe actuar como observador y que esté marcado como no constante.");	      
        
#ifdef PRACTICA1
    
    cf.method({"operator const char *"}, {{"?"}}, "Fecha", {"?"}, {false}, 
    	"El operador de conversión a cadena de bajo nivel debe ser retirado.");	 
#endif

    cf.method({"operator+", "operator-", "operator+=", "operator-="}, 
    	      {{"?"}, {"?"}, {"?"}, {"?"}}, "Fecha", 
    	      {"?", "?", "?", "?"}, {true, true, true, true}, 
	"Revisa el lugar en el que  deberían incluirse los operadores de suma y resta de días.");
  ...
  if(cc.findClass({"Cadena"})) {
    ...
    cc.method({"operator="}, {{"const char *"}}, "Cadena", {"noconst"}, {true}, "No se detecta" 
		    " el operador de asignación con conversión desde cadena"
		    " de bajo nivel.");
		    
    cc.method({"operator+"}, {{"?"}}, "Cadena", {"const"}, {false}, 
    	"Considera incluir el operador de concatenación de Cadena como una función externa.");	
    	
    cc.method({"operator+="}, {{"?"}}, "Cadena", {"?"}, {true}, 
    	"Revisa el lugar en el que debería incluirse el operador de suma (concatenación) con asignación.");	    
    	
    cc.method({"operator const char *"}, {{"?"}}, "Cadena", {"noconst"}, {false},
        "Revisa si tienes un método u operador que debe actuar como observador y que esté marcado como no constante.");	
